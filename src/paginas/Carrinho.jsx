...
export default function Carrinho({ carr, itens, setItens, opcionaisData }) {
    ...
    const valStr = String(item.produto.valor || '0').replace(',', '.');
    let valorBase = parseFloat(valStr) || 0;
    ...
    Object.entries(item.opcionais || {}).forEach(([idStr, qtd]) => {
        ...
        if (opc && qtd > 0) {
            const opcValStr = String(opc.valor || '0').replace(',', '.');
            let valorOpc = parseFloat(opcValStr) || 0;
    ...
    const handleConfirmarAgendamento = async (e) => {
        ...
        try {
            ...
            const { data: solicitacao, error: errorSol } = await supabase
                ...
                .insert([{
                    codigo_pedido: `#AG-${Math.floor(1000 + Math.random() * 9000)}`,
            ...
            const codFormatado = solicitacao.codigo_pedido || `#AG-${String(solicitacao.id).padStart(4, '0')}`;
            const mensagemWhatsApp = encodeURIComponent(`Olá! Solicitei um agendamento de número ${codFormatado}`);
    ...
    return (
        <section className='fixed inset-0 z-50 flex justify-center items-center bg-black/40 backdrop-blur-xs px-4 overscroll-contain'>
            <div className='w-full max-w-[420px] max-h-[85vh] rounded-3xl flex flex-col bg-white ring-1 ring-[#C5A059]/50 shadow-2xl p-4 overflow-hidden relative overscroll-contain'>
                ...
                {enviando && (
                    <div className='absolute inset-0 z-55 flex flex-col items-center justify-center bg-white/95 gap-3'>
                        ...
                        <p className='text-sm text-gray-700 font-semibold font-cinzel'>Enviando agendamento...</p>
                    </div>
                )}
                ...
                <div className='w-full flex justify-between items-center pb-2 border-b border-gray-100'>
                    <button 
                        ...
                        className='h-8 w-8 hover:scale-110 active:scale-100 flex justify-center items-center pb-0.5 rounded-full bg-[#C5A059]/20 text-[#C5A059] transition-all cursor-pointer font-bold'
                    >
                    ...
                    {!mostrarFormulario && historico.length > 0 ? (
                        <button 
                            ...
                            className='text-[10px] font-bold text-[#C5A059] hover:underline cursor-pointer bg-[#C5A059]/10 rounded-full px-2.5 py-1'
                            ...
                            <div key={hist.id} className='bg-gray-50 rounded-2xl p-3.5 border border-gray-200/70 flex flex-col gap-2.5 shadow-2xs'>
                                ...
                                <div className='flex flex-col gap-1'>
                                    {(hist.itensCompletos || []).map((item, idx) => (
                                        <div key={idx} className='text-xs text-gray-700 font-medium flex items-center justify-between'>
                                            ...
                                            <span className='text-[11px] font-bold text-[#C5A059]'>R$ {item.produto?.valor}</span>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className='flex justify-between items-center pt-2 border-t border-gray-200/50'>
                                    ...
                                    <button 
                                        ...
                                        className='px-3 py-1.5 bg-[#C5A059] hover:bg-[#B88E3E] text-white text-xs font-bold rounded-xl shadow-xs hover:scale-105 active:scale-95 transition-all cursor-pointer flex items-center gap-1'
                                    >
                        ...
                        <div className='flex-1 overflow-y-auto overscroll-contain no-scrollbar py-3 flex flex-col gap-4 permitir-scroll'>
                            {itens.length === 0 ? (
                                <div className='flex flex-col items-center justify-center py-6 gap-3'>
                                    <div className='w-16 h-16 rounded-full border-2 border-[#C5A059]/40 bg-[#0F172A] flex items-center justify-center overflow-hidden p-1 shadow-md'>
                                        <img src={AlbertSalesLogo} className='w-full h-full object-cover' />
                                    </div>
                                    ...
                                    {historico.length > 0 && (
                                        <div className='mt-2 w-full pt-4 border-t border-gray-100 flex flex-col gap-2'>
                                            ...
                                            <button 
                                                ...
                                                className='w-full py-2 bg-gray-50 hover:bg-gray-100 text-[#C5A059] border border-[#C5A059]/40 font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5'
                                            >
                                ...
                                itens.map((item) => (
                                    <div key={item.idEscolha} className='bg-gray-50 rounded-2xl p-3 border border-gray-100 flex flex-col gap-2 relative shadow-sm'>
                                        ...
                                        <div className='flex items-center gap-3'>
                                            <img src={item.produto?.img || AlbertSalesLogo} className='w-12 h-12 bg-white rounded-xl shadow-sm object-cover border border-gray-100' />
                                            <div className='flex flex-col text-left'>
                                                ...
                                                <span className='text-xs font-bold text-[#C5A059] mt-0.5'>
                        ...
                        {itens.length > 0 && (
                            <div className='pt-3 border-t border-gray-100 flex flex-col gap-3'>
                                <div className='flex justify-between items-center text-sm font-bold text-gray-800 px-1'>
                                    ...
                                    <span className='text-lg text-[#C5A059]'>R$ {calcularTotalGeral().toFixed(2).replace('.', ',')}</span>
                                </div>
                                <button 
                                    onClick={() => setMostrarFormulario(true)}
                                    className='w-full py-2.5 bg-[#C5A059] hover:bg-[#B88E3E] text-white font-bold text-sm rounded-xl shadow-md hover:scale-[1.02] active:scale-95 transition-all cursor-pointer'
                                >
